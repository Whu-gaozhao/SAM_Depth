...
class SpatialPathDepth(nn.Module):
    def __init__(self, *args, **kwargs):
        super(SpatialPathDepth, self).__init__()
        self.conv1 = ConvBNReLU(1, 64, ks=7, stride=2, padding=3)
        self.conv2 = ConvBNReLU(64, 64, ks=3, stride=2, padding=1)
        self.conv3 = ConvBNReLU(64, 64, ks=3, stride=2, padding=1)
        self.conv_out = ConvBNReLU(64, 128, ks=1, stride=1, padding=0)
        self.init_weight()
...
class ABCNet(nn.Module):
    def __init__(self, band=3, num_classes=6, pretrained=True):
        super(ABCNet, self).__init__()
        self.name = 'ABCNet'
        self.cp = ContextPath()
        self.sp = SpatialPath()
        self.spd = SpatialPathDepth()
        self.fam = FeatureAggregationModule(256, 256)
        self.conv_out = Output(256, 256, num_classes, up_factor=8)
        if self.training:
            self.conv_out16 = Output(128, 64, num_classes, up_factor=8)
    ...
    def forward(self, x):
        ...
        feat_cp8, feat_cp16, feat_cp32 = self.cp(color)
        # feat_spd = self.spd(depth)
        feat_sp = self.sp(color)
